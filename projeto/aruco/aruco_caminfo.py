...
from sensor_msgs.msg import Image, CompressedImage, LaserScan, CameraInfo
...
def recebe_caminfo(msg):
	global camera_matrix
	global camera_distortion
	camera_matrix = np.array(msg.K).reshape(3, 3)
	camera_distortion = np.array(msg.D)
...
if __name__=="__main__":
	...
	topico_camera_info = "/camera/camera_info"
	#topico_imagem = "/camera/color/image_raw/compressed" #robo real
	#topico_camera_info = "/camera/color/camera_info" #robo real
	recebe_imagem = rospy.Subscriber(topico_imagem, CompressedImage, roda_todo_frame, queue_size=4, buff_size = 2**24)
	recebe_caminfo = rospy.Subscriber(topico_camera_info, CameraInfo, recebe_caminfo)
